...
public class LegacyParticleUpdater : ScriptableWizard
{
   ...
   // private ParticleEmitter[] prefabs;
    ...
    void OnWizardUpdate()
    {
        ...
        This script is not support in this version, please run this script using a Unity version between 2017.4 and 2018.2.";
#else
        helpString = @"This Script adds ParticleSystem and ParticleSystemRenderer Components to all GameObjects that contain Legacy Particle Components.
        Legacy Particle System Components can be deleted, disabled, or preserved for comparison.
        Please run this script using Unity 2017.4 or newer.";
#endif
    }
    ...
    void Upgrade(ParticleEmitter emitter)
    {
        ...
        if (animator != null)
        {
            ...
            if (linearForce.sqrMagnitude > Mathf.Epsilon)
            {
                forceOverLifetime.enabled = true;
                forceOverLifetime.x = linearForce.x;
                forceOverLifetime.y = linearForce.y;
                forceOverLifetime.z = linearForce.z;
            }
            else if (rndForce.sqrMagnitude > Mathf.Epsilon)
            {
                forceOverLifetime.enabled = true;
                forceOverLifetime.randomized = true;
                forceOverLifetime.x = ConvertToMinMaxCurve(-rndForce.x, rndForce.x);
                forceOverLifetime.y = ConvertToMinMaxCurve(-rndForce.y, rndForce.y);
                forceOverLifetime.z = ConvertToMinMaxCurve(-rndForce.z, rndForce.z);
            }
